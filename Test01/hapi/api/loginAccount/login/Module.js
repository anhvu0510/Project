    ...
    try{
        ...
        if((status === true) && (reply !== 'LOCKED_SPAM') && (reply !== 'LOCKED') && (replyUser !== 'LOCKED')){
            ...
            if(_.isNil(replyUser)){
                await Redis.setCache(username, 1)
            }
            //Get timeout
            ...
            if(reply === 20 && timeout > 0){
                await Redis.setCache(IP,"LOCKED_SPAM",TIME_LOCK_SPAM_LOGIN)
            ...
            //Check Username
            if(replyUser === 2){
                await Redis.setCache(username,"LOCKED",TIME_LOCK_LOGIN_US);
                await Redis.setCache(IP,"LOCKED",TIME_LOCK_LOGIN_IP)
                ...
                await Redis.setCache(username,Number(++replyUser));
            }
